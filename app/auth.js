  ...
  static getAccessToken = async () => {
    try {
      ...
      if (userInfo.access_token && Date.now() < userInfo.expires_on) {
        ...
        const data = await refreshToken(Config.Data.apiConfig.refresh, userInfo.login, userInfo.refresh_token);

        if (data.access_token) {
          await Storage.setItem('userInfo', JSON.stringify({login: userInfo.login,...data}));
